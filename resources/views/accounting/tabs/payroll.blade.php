<div x-data="{ payrollTab: 'payrolls' }">
  <div class="mb-6 flex space-x-4 border-b border-gray-200 ">
    <button @click="payrollTab = 'payrolls'" :class="{ 'border-b-2 border-mango text-gray-900 ': payrollTab === 'payrolls', 'text-gray-500 hover:text-gray-700': payrollTab !== 'payrolls' }" class="pb-2 font-bold px-2">Payrolls</button>
    <button @click="payrollTab = 'employees'" :class="{ 'border-b-2 border-mango text-gray-900 ': payrollTab === 'employees', 'text-gray-500 hover:text-gray-700': payrollTab !== 'employees' }" class="pb-2 font-bold px-2">Employees</button>
    <button @click="payrollTab = 'attendance'" :class="{ 'border-b-2 border-mango text-gray-900 ': payrollTab === 'attendance', 'text-gray-500 hover:text-gray-700': payrollTab !== 'attendance' }" class="pb-2 font-bold px-2">Attendance</button>
    <button @click="payrollTab = 'advances'" :class="{ 'border-b-2 border-mango text-gray-900 ': payrollTab === 'advances', 'text-gray-500 hover:text-gray-700': payrollTab !== 'advances' }" class="pb-2 font-bold px-2">Advances</button>
  </div>
  ...
  <div x-show="payrollTab === 'payrolls'" class="space-y-6">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 ">
        ...
        <form action="{{ route('accounting.generatePayroll') }}" method="POST" class="space-y-4">
          ...
          <div>
            ...
            <select name="employee_id" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" name="month" value="{{ date('n') }}" min="1" max="12" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">Year</label>
              <input type="number" name="year" value="{{ date('Y') }}" min="2000" max="2100" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
            </div>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">Bonus (+)</label>
              <input type="number" name="bonus" value="0" min="0" step="0.01" class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">Incentives (+)</label>
              <input type="number" name="incentives" value="0" min="0" step="0.01" class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
            </div>
          </div>
      ...
      <!-- Payroll History -->
      <div class="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-100 ">
              ...
              <tr>
                ...
                <td class="py-4">
                  ...
                  <div class="font-bold text-gray-900 ">Net: Rs.{{ number_format($pr->net_payable) }}</div>
                </td>
                <td class="py-4">
                  @if($pr->status === 'paid')
                ...
                <td class="py-4 text-right">
                  ...
                  <form action="{{ route('accounting.payPayroll') }}" method="POST" class="inline-flex items-center gap-2">
                    ...
                    <input type="date" name="date" value="{{ date('Y-m-d') }}" required class="rounded-lg border-gray-300 text-xs py-1.5  w-32">
                    <select name="account_id" required class="rounded-lg border-gray-300 text-xs py-1.5  w-24">
                      @foreach($data['accounts'] as $acc)
                        <option value="{{ $acc->id }}">{{ $acc->name }}</option>
                      @endforeach
                    </select>
                    <button type="submit" class="bg-gray-900 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-gray-800">Pay</button>
                  </form>
                  @endif
                </td>
              </tr>
      ...
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 ">
        ...
        <form action="{{ route('accounting.storeEmployee') }}" method="POST" class="space-y-4">
          ...
          <div>
            ...
            <input type="text" name="name" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          </div>
          <div>
            <label class="block text-sm font-bold text-gray-700 mb-1">Designation</label>
            <input type="text" name="designation" class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          </div>
          <div>
            <label class="block text-sm font-bold text-gray-700 mb-1">Base Salary</label>
            <input type="number" name="base_salary" min="0" step="0.01" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">Phone</label>
              <input type="text" name="phone" class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">Join Date</label>
              <input type="date" name="join_date" class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
            </div>
          </div>
          <div>
            <label class="block text-sm font-bold text-gray-700 mb-1">Status</label>
            <select name="status" class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
              <option value="active">Active</option>
              <option value="inactive">Inactive</option>
            </select>
          </div>
          <button type="submit" class="w-full bg-gray-900 text-white font-bold py-3 rounded-xl hover:bg-gray-800 transition-colors">
            Save Employee
          </button>
        </form>
      </div>
  ...
  <div x-show="payrollTab === 'attendance'" class="space-y-6" style="display: none;">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 ">
        ...
        <form action="{{ route('accounting.storeAttendance') }}" method="POST" class="space-y-4">
          ...
          <div>
            ...
            <input type="date" name="date" value="{{ date('Y-m-d') }}" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          </div>
          <div>
            <label class="block text-sm font-bold text-gray-700 mb-1">Employee</label>
            <select name="employee_id" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          ...
          <div>
            ...
            <input type="text" name="notes" class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          </div>
          <button type="submit" class="w-full bg-gray-900 text-white font-bold py-3 rounded-xl hover:bg-gray-800 transition-colors">
            Mark Attendance
          </button>
      ...
      <div class="bg-blue-50 p-6 rounded-2xl border border-blue-100 ">
        <h3 class="text-lg font-black text-blue-800 mb-2">How Attendance Works</h3>
      ...
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 ">
        ...
        <form action="{{ route('accounting.storeAdvance') }}" method="POST" class="space-y-4">
          ...
          <div>
            ...
            <input type="number" name="amount" min="1" step="0.01" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          </div>
          <div>
            <label class="block text-sm font-bold text-gray-700 mb-1">Date</label>
            <input type="date" name="date" value="{{ date('Y-m-d') }}" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          ...
          <div>
            ...
            <select name="account_id" required class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
              @foreach($data['accounts'] as $acc)
          ...
          <div>
            ...
            <input type="text" name="description" class="w-full rounded-xl border-gray-300  focus:ring-mango focus:border-mango">
          </div>
          <button type="submit" class="w-full bg-gray-900 text-white font-bold py-3 rounded-xl hover:bg-gray-800 transition-colors">
            Record Advance
          </button>
