<aside class="fixed inset-y-0 left-0 z-50 bg-white border-r border-gray-100 transform transition-all duration-300 ease-in-out md:translate-x-0 md:static md:inset-0 flex flex-col shadow-[4px_0_24px_rgb(0,0,0,0.02)] " 
  :class="{
 ...
 <!-- Sidebar Header (Logo) -->
 <div class="h-[72px] flex items-center border-b border-gray-50/50 shrink-0" :class="sidebarCollapsed ? 'px-3 justify-center' : 'px-6'">
  <a href="{{ route('dashboard') }}" class="flex items-center gap-3 group" :class="sidebarCollapsed ? 'justify-center' : 'w-full'">
   ...
   <h1 x-show="!sidebarCollapsed" x-transition.opacity class="text-xl font-black tracking-tight text-gray-900 whitespace-nowrap">Mission <span class="text-mango">Control</span></h1>
  </a>
  <button @click="sidebarOpen = false" x-show="!sidebarCollapsed" class="md:hidden text-gray-400 hover:text-gray-900 transition-colors p-1 rounded-lg hover:bg-gray-100 ml-auto">
 ...
 <div class="flex-1 overflow-y-auto overflow-x-hidden no-scrollbar py-6 space-y-8" :class="sidebarCollapsed ? 'px-2' : 'px-4'">
  ...
  <div>
   ...
   <div class="space-y-1">
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3 py-2.5 rounded-xl font-bold text-sm transition-all {{ request()->routeIs('dashboard') ? 'bg-mango/10 text-mango shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'" title="Dashboard">
    ...
    @if(auth()->user()->hasPermission('analytics'))
     <a href="{{ route('analytics.index') }}" class="flex items-center gap-3 py-2.5 rounded-xl font-bold text-sm transition-all {{ request()->routeIs('analytics.*') ? 'bg-mango/10 text-mango shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'" title="Analytics">
  ...
  @if(auth()->user()->hasPermission('orders') || auth()->user()->hasPermission('products') || auth()->user()->hasPermission('categories') || auth()->user()->hasPermission('customers'))
   ...
   <div>
    ...
    <div class="space-y-1">
     ...
     <a href="{{ route('products.index') }}" class="flex items-center gap-3 py-2.5 rounded-xl font-bold text-sm transition-all {{ request()->routeIs('products.*') ? 'bg-gray-900 text-white shadow-lg shadow-gray-900/20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'" title="Products">
   ...
   <!-- Marketing Section -->
   <div>
    <div x-show="!sidebarCollapsed" class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 px-3">Marketing</div>
    <div class="space-y-1">
     <a href="{{ route('facebook-inbox.index') }}" class="flex items-center gap-3 py-2.5 rounded-xl font-bold text-sm transition-all {{ request()->routeIs('facebook-inbox.*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'" title="Facebook Inbox">
   ...
   <div>
    ...
    <div class="space-y-1">
     <a href="{{ route('pathao.index') }}" class="flex items-center gap-3 py-2.5 rounded-xl font-bold text-sm transition-all {{ request()->routeIs('pathao.*') ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'" title="Pathao Manager">
     ...
     <a href="{{ route('rider_comments.index') }}" class="flex items-center justify-between py-2.5 rounded-xl font-bold text-sm transition-all {{ request()->routeIs('rider_comments.*') ? 'bg-mango/10 text-mango shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'" title="Rider Comments">
      <div class="flex items-center gap-3">
       <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('rider_comments.*') ? 'text-mango' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
  ...
  @if(auth()->user()->hasPermission('accounting') || auth()->user()->hasPermission('purchases') || auth()->user()->hasPermission('expenses'))
   ...
   <div x-data="{ open: {{ request()->routeIs('accounting.*') || request()->routeIs('purchases.*') || request()->routeIs('expenses.*') ? 'true' : 'false' }} }">
    <template x-if="!sidebarCollapsed">
     <button @click="open = !open" class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl font-bold text-sm transition-all text-gray-500 hover:bg-gray-50 hover:text-gray-900 group">
      <div class="flex items-center gap-3">
    ...
    <div x-show="open || sidebarCollapsed" x-collapse class="space-y-1" :class="sidebarCollapsed ? '' : 'pl-11 pr-3 pt-2'">
     ...
     <a href="{{ route('accounting.index') }}" class="block py-2 text-sm font-bold transition-colors {{ request()->routeIs('accounting.*') ? 'text-mango' : 'text-gray-400 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'text-center text-[10px]' : ''" title="Accounting">
     ...
     <a href="{{ route('purchases.index') }}" class="block py-2 text-sm font-bold transition-colors {{ request()->routeIs('purchases.*') ? 'text-mango' : 'text-gray-400 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'text-center' : ''" title="Purchases">
     ...
     <a href="{{ route('expenses.index') }}" class="block py-2 text-sm font-bold transition-colors {{ request()->routeIs('expenses.*') ? 'text-mango' : 'text-gray-400 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'text-center' : ''" title="Expenses">
  ...
  @if(auth()->user()->hasPermission('settings') || auth()->user()->hasPermission('users'))
   ...
   <div x-data="{ open: {{ request()->routeIs('settings.*') || request()->routeIs('users.*') ? 'true' : 'false' }} }">
    ...
    <div x-show="open || sidebarCollapsed" x-collapse class="space-y-1" :class="sidebarCollapsed ? '' : 'pl-11 pr-3 pt-2'">
     ...
     <a href="{{ route('settings.index', ['tab' => 'frontend']) }}" class="block py-2 text-sm font-bold transition-colors {{ request()->routeIs('settings.*') ? 'text-mango' : 'text-gray-400 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'text-center' : ''" title="Settings">
      <span x-show="!sidebarCollapsed">General Settings</span>
      <svg x-show="sidebarCollapsed" class="w-5 h-5 mx-auto {{ request()->routeIs('settings.*') ? 'text-mango' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
     </a>
     <a href="{{ route('activity-log.index') }}" class="block py-2 text-sm font-bold transition-colors {{ request()->routeIs('activity-log.*') ? 'text-mango' : 'text-gray-400 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'text-center' : ''" title="Activity Log">
     ...
     <a href="{{ route('users.index') }}" class="block py-2 text-sm font-bold transition-colors {{ request()->routeIs('users.*') ? 'text-mango' : 'text-gray-400 hover:text-gray-900 ' }}" :class="sidebarCollapsed ? 'text-center' : ''" title="Staff & Roles">
 ...
 <!-- Sidebar Footer -->
 <div class="border-t border-gray-50/50 " :class="sidebarCollapsed ? 'p-2' : 'p-4'">
  <div class="bg-gray-50 rounded-2xl flex items-center" :class="sidebarCollapsed ? 'p-2 justify-center' : 'p-4 justify-between'">
