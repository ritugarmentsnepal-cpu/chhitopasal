  ...
  @if(auth()->user()->hasPermission('orders') || auth()->user()->hasPermission('products') || auth()->user()->hasPermission('categories') || auth()->user()->hasPermission('customers'))
   ...
   <div>
    ...
    <div class="space-y-1">
     ...
     @endif
     @if(auth()->user()->hasPermission('orders') && Route::has('mockups.index'))
     {{-- PHASE-3.2: Mockup Studio promoted to a top-level daily tool --}}
     <a href="{{ route('mockups.index') }}" class="flex items-center gap-3 py-2.5 rounded-xl font-semibold text-sm transition-all relative {{ request()->routeIs('mockups.*') ? 'bg-indigo-500/15 text-indigo-400' : 'text-white/50 hover:bg-white/5 hover:text-white/80' }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'" title="Mockup Studio">
      @if(request()->routeIs('mockups.*'))
      <div class="absolute left-0 w-1 h-6 gradient-bg-vibrant rounded-r-full" :class="sidebarCollapsed ? 'hidden' : ''"></div>
      @endif
      <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('mockups.*') ? 'text-indigo-400' : 'text-white/30' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
    ...
    <div class="space-y-1">
     ...
     <a href="{{ route('support-tickets.index') }}" class="flex items-center justify-between py-2.5 rounded-xl font-semibold text-sm transition-all {{ request()->routeIs('support-tickets.*') ? 'bg-orange-500/15 text-orange-400' : 'text-white/50 hover:bg-white/5 hover:text-white/80' }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'" title="Support Tickets">
      ...
      @if(($openTicketsCount ?? 0) > 0)
       <span x-show="!sidebarCollapsed" class="bg-red-500 text-white py-0.5 px-2 rounded-full text-[10px] font-black">{{ $openTicketsCount }}</span>
      @endif
     </a>
    </div>
   </div>
    ...
    <div class="space-y-1">
     ...
     <a href="{{ route('rider_comments.index') }}" class="flex items-center justify-between py-2.5 rounded-xl font-semibold text-sm transition-all {{ request()->routeIs('rider_comments.*') ? 'bg-white/10 text-white' : 'text-white/50 hover:bg-white/5 hover:text-white/80' }}" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-3'" title="Rider Comments">
      ...
      @if(($unreadCommentsCount ?? 0) > 0)
       <span x-show="!sidebarCollapsed" class="bg-red-500 text-white py-0.5 px-2 rounded-full text-[10px] font-black">{{ $unreadCommentsCount }}</span>
      @endif
     </a>
    </div>
   </div>
