<div x-data="{ searchFocused: false }" @keydown.meta.k.window.prevent="$refs.globalSearch.focus()" class="bg-white/80 backdrop-blur-xl border-b border-gray-100/50 sticky top-0 z-40 h-[72px] flex items-center justify-between px-4 sm:px-6 lg:px-8">
 ...
 <div class="flex items-center gap-4 flex-1">
  ...
  <form action="{{ route('orders.index') }}" method="GET" class="hidden sm:flex items-center max-w-md w-full relative group">
   ...
   <input x-ref="globalSearch" type="text" name="search" class="block w-full pl-10 pr-14 py-2.5 border border-gray-200/60 rounded-xl leading-5 bg-gray-50/80 placeholder-gray-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary/30 sm:text-sm transition-all font-medium" placeholder="Search orders, products, customers...">
   <div class="absolute inset-y-0 right-0 pr-2.5 flex items-center pointer-events-none">
    <kbd class="hidden sm:inline-flex items-center border border-gray-200/60 rounded-lg px-2 py-0.5 text-[10px] font-bold text-gray-400 bg-white/80 shadow-sm gap-0.5">⌘K</kbd>
   </div>
  </form>
 </div>
