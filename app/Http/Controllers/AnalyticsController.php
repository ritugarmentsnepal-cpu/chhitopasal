...
class AnalyticsController extends Controller
{
    public function index(Request $request)
    {
        ...
        try {
            ...
            // Analytics tables may not exist yet (migration not run)
            \Illuminate\Support\Facades\Log::warning('Analytics tables not available: ' . $e->getMessage());

            $totalVisitors = $sessionsWithProductView = $sessionsWithAddToCart = $sessionsWithCheckout = 0;
            $totalOrders = 0;
            $totalRevenue = 0;
            $conversionRate = 0;
            $bounceRate = 0;
            $topViewedProducts = collect();
            $topCartProducts = collect();
            $categoryBreakdown = collect();
            $campaigns = collect();
            $funnelData = ['labels' => ['Visitors', 'Explored Products', 'Added to Cart', 'Initiated Checkout', 'Orders'], 'data' => [0, 0, 0, 0, 0]];
